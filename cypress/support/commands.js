...
Cypress.Commands.add('getUsers', (options = {}) => {
  const baseUrl = options.baseUrl ?? Cypress.env('API_BASE_URL') ?? 'https://jsonplaceholder.typicode.com'
  const route = options.route ?? Cypress.env('API_ROUTE') ?? '/users'
  ...
  return cy.request('GET', `${baseUrl}${route}`).then(({ status, body }) => {
    ...
    cy.log(
      `**getUsers** - ${users.length} usuarios processados${
        tldFilter ? ` (filtro: .${tldFilter})` : ''
      }`
    )
